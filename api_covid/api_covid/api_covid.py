...
def users():
	try:
		...
		cur.execute("SELECT * FROM mydb2.tb_usuario;")
...
def cities():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.city;")
...
def country():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.country;")
...
def state():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.state;")
...
def hospital():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.tb_hospital;")
...
def pathology():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.tb_patologia;")
...
def symptoms():
	try:
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("SELECT * FROM mydb2.tb_sintomas;")
...
def api_filter_user():
	...
	if not (idt_usuario or nme_usuario or ida_usuario or address or city_id or pto_user):
...
def api_filter_hospital():
	...
	bairro_hospital  = query_parameters.get("address")
	city_id     = query_parameters.get("city.id")
...
@app.route('/api/v1/insert/usuarios', methods=['GET', 'POST'])
def api_insert_user():
	try:
		nme_usuario = 	request.form['nme_usuario'] 
		ida_usuario = 	request.form['ida_usuario']
		psw_usuario =	request.form['ida_usuario']
		address 	=	request.form['address']
		conn = mysql.connect()
		cur = conn.cursor(pymysql.cursors.DictCursor)
		cur.execute("INSERT INTO mydb2.tb_usuario(nme_usuario,ida_usuario,address,psw_usuario) VALUES (%s,%s,%s,%s)",(nme_usuario,ida_usuario,address,psw_usuario))
		conn.commit()
		cur.close()
		conn.close()
		return "Registered"
