...
axios.get('http://localhost/api/v1/sintomas/all')
...
document.getElementById('addSintoma').addEventListener('submit', saveSintoma);
    ...
    function saveSintoma(e){
        ...
        axios.post("http://localhost/api/v1/insert/sintoma?idt_usuario=" + localStorage.id,{
    ...
    function saveTemperatura(e){
        ...
        axios.post("http://localhost/api/v1/insert/temperatura?idt_usuario=" + localStorage.id,{
...
function loadSintomas(){
        ...
        const table = document.getElementById('tableRow');
          ...
          axios.get('http://localhost/api/v1/resources/sintomas_usuario?idt_usuario=' + localStorage.id)
          .then(function (response){
            for(var i = 0; i < response.data.length; i++){
          ...
          axios.get('http://localhost/api/v1/resources/temperatura?idt_usuario=' + localStorage.id)
          .then(function (response){
            for(var i = 0; i < response.data.length; i++){
